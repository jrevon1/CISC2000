...
// freeBoard frees up the space allocated to the 
// dynamic array to prevent memory leaks.
...
// createNewBoard - creates a new Galton board
// precondition: none
// postcondition: a dynamically allocated array of size "rows", where the first row
// contains i elements, the second contains i+1 elements, and so on until i is equal
// to "rows".

// Note: all elements in the array are set to 0 initially
int** createNewBoard(int rows)
{
  // Declare an integer pointer to a pointer "board" and set that to a pointer
  // to the memory location of a new dynamic integer array with a size of "rows"
...
// tests to perform.
// postcondition: a dynamically allocated two-dimensional array filled with values
// based on the number of tests and rows.
void runSimulations(int *board[], int rows, int nTests)
{
	int i, j;
