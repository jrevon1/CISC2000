/* simple_test.cpp
 * This program: Creates paychecks for employees based on their type (hourly, salaried, admin, or commission).
 * The user is prompted to:
 *      - Enter the number of employees they would like to enter payment details for
 *      - Select the type of each employee
 *      - Enter differing details for each employee based on that employee's selected type
 * The program then stores that data for each employee, and finally prints a unique check stub based on their selected type
 * Author: Jordan Revon
...
int main( )
{
	...
	for (int i=0; i<numberEmployees; i++)
  {
    ...
    cin >> type;
      ...
      CommissionEmployee * commish = new CommissionEmployee();

			// Prompt for input
      commish->promptInput(cin, cout);

			// Add to the lists
      employeePtrs.push_back(commish);
      employeeSlices.push_back(*commish);
		}
   }
